...
<div class="card-tools" style="float: right;">
         <a href="appointments" class="btn btn-primary"><i class="fas fa-list-ul"></i> Lists of Appointments</a>
          <!--<a href="list-trash-supply" class="btn btn-success"><i class="fas fa-trash-restore"></i> Recycle Bin</a>--> 
...
<div class="card">
             <div class="card-body">
<table id="appointmentTransactionTable" class="table table-bordered table-striped">
                    <thead>
<tr>
    <!--<th>#</th>-->
    <th>Created At</th>
...
<tbody>
   @foreach($appointment_transactions as $key => $transaction)     
        <tr data-entry-id="{{ $transaction->id }}" @if($loop->first) class="bg-info" @endif>    
            <!--<td>{{$loop->iteration}}</td> ->diffForHumans()-->
            <td>{{($transaction->created_at)->isoFormat('MMMM D, YYYY (h:mm a)')?? ''}}</td>
            
            <td>{{$transaction->appointment->patient->firstname}} {{$transaction->appointment->patient->middlename}} {{$transaction->appointment->patient->lastname}}</td>
            <td>{{$transaction->appointment->service->service ?? ''}}</td>
...
<script>
   ...
   // var table = $('# your selector').DataTable();
        var table = $('#appointmentTransactionTable').DataTable({
            bSort: false,
            ...
            buttons: [
                    ...
                    $(win.document.body)
                        ...
                        .prepend(
                            ...
                            '<h4 style="margin-top:20px; margin-left:480px";>List of Appointments Transactions</h4>',
                        );
        ...
        }).buttons().container().appendTo('#appointmentTransactionTable_wrapper .col-md-6:eq(0)');
