...
@section('title')
Details of {{ $stock_medicine->medicine->medicine_name}}
@endsection
@section('css')
    ...
    <div class="card-body">
        <div class="form-group">
            <div class="form-group" style="float: right;">
                <a class="btn btn-secondary" href="/medicine-stock"><i class="fas fa-reply"></i> Back to inventory</a>
                <button type="button" value="{{$stock_medicine->id}}" class="btn btn-warning editbtn"><i class="fas fa-edit"></i> Edit Medicine Stock</button>
            </div>
            <table class="table table-bordered table-striped">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            {{ $stock_medicine->medicine->medicine_name ?? '' }}
                        </td>
                    </tr>
                    <tr>
                        <th>
                        ...
                        <td>
                            {{ $stock_medicine->medicine->danger_level ?? '' }}
                        </td>
                    </tr>
                    <tr>
                        <th>
                        ...
                        <td>
                            {{ $stock_medicine->current_stock }}
                        </td>
                    </tr>
                </tbody>
            </table>
            <h4 class="text-center">
                History of {{ $stock_medicine->medicine->medicine_name }}
                @if(count($stock_medicine->medicine->transactions) == 0)
                    is empty
                @endif
            </h4>
            @if(count($stock_medicine->medicine->transactions) > 0)
                <table id="medicineDetaisTransactionTable" class="table  table-bordered table-striped display compact" style="width:75%; margin: auto;">
                    <thead>
                        <tr>
                            <th>Created At</th>
                            <th>User</th>
                        ...
                        @foreach($stock_medicine->medicine->transactions as $transaction)
...
<!-- Modal -->
<div class="modal fade" id="editStockMedicine" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Stock Details of {{ $stock_medicine->medicine->medicine_name}}</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form method="POST" action="{{url('update-medicine-stock')}}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
...
<script>
    $(document).ready(function () {
        ...
        $(document).on('click', '.editbtn', function () {
            var stock_medicine_id = $(this).val();
            //alert(stock_supply);
            $('#editStockMedicine').modal('show');

            $.ajax({
                type: "GET",
                url: "/edit-medicine-stock/" + stock_medicine_id,
                success: function (response) {
                    console.log(response);
                    $('#current_stock').val(response.stock_medicine.current_stock);
                    $('#consumed').val(response.stock_medicine.consumed);
                    $('#total').val(response.stock_medicine.total);
                    $('#stock_medicine_id').val(stock_medicine_id);
                }
            });
        });
   // var table = $('# your selector').DataTable();
        var table = $('#medicineDetaisTransactionTable').DataTable({
            bSort: false,
            //dom:'lBfrtip',
                //'flrtipB',
                //select: true,
            ...
            buttons: [
                    ...
                    $(win.document.body)
                        ...
                        .prepend(
                            ...
                            '<h4 style="margin-top:20px; margin-left:480px";>List of Medicines</h4>',
                        );
...
@if(Session::has('stock_medicine_updated'))
           <script>
              Swal.fire(
              'Good job!',
              '{!! Session::get('stock_medicine_updated') !!}',
              'success'
            )
           </script>              
@endif
