...
<div class="card">
             <div class="card-body">
<table id="medicineTransactionTable" class="table table-bordered table-striped">
                    <thead>
<tr>
    <!--<th>#</th>-->
    <th>Created At</th>
...
<tbody>
   @foreach($medicine_transactions as $key => $transaction)     
        <tr data-entry-id="{{ $transaction->id }}" @if($loop->first) class="bg-info" @endif>    
            ...
            <td>{{$transaction->medicine->medicine_name ?? ''}}</td>
...
<script>
   ...
   // var table = $('# your selector').DataTable();
        var table = $('#medicineTransactionTable').DataTable({
            bSort: false,
            //dom:'lBfrtip',
                //'flrtipB',
                //select: true,
        ...
        }).buttons().container().appendTo('#medicineTransactionTable_wrapper .col-md-6:eq(0)');
