...
<div class="card-tools" style="float: right;">
          ...
          <a href="supply-stock" class="btn btn-secondary"><i class="fas fa-dolly-flatbed"></i> Inventory of Supplies</a>
...
<tbody>
     @foreach($supplies as $supply)
        <tr>
            ...
            @if($supply->picture != NULL)
            <td><img src="{{asset('images')}}/{{$supply->picture}}"  style="max-width:60px;"/></td>
            @endif
            @if($supply->picture == NULL)
            <td>No image inserted</td>
            @endif
           <!--<td>{{$supply->quantity}}</td>-->
           <td>{{$supply->unit}}</td>
           <td>{{$supply->danger_level}}</td>
           <td>{{date('F d, Y', strtotime($supply->date_received))}}</td>
            <td>
                <a href="/edit-supplies/{{$supply->id}}" class="btn btn-sm btn-warning"> <i class="fa fa-edit"></i> Edit</a>
                
                <button type="button" class="btn btn-sm btn-danger delete-btn" data-id="{{$supply->id}}"><i class="fa fa-trash"></i> Delete </button> 
...
<script>
   ...
   // var table = $('# your selector').DataTable();
        ...
        }).buttons().container().appendTo('#supplytable_wrapper .col-md-6:eq(0)');
