...
@section('title')
Details of {{ $stock_supply->supply->supply_name}}
@endsection
@section('css')
    ...
    <div class="card-body">
        <div class="form-group">
            <div class="form-group" style="float: right;">
                <a class="btn btn-secondary" href="/supply-stock"><i class="fas fa-reply"></i> Back to inventory</a>
                <button type="button" value="{{$stock_supply->id}}" class="btn btn-warning editbtn"><i class="fas fa-edit"></i> Edit Supply Stock</button>
                <!--<a class="btn btn-warning" href=""><i class="fas fa-edit"></i> Edit Supply Stock</a>data-toggle="modal" data-target="#editStockSupply"-->
            </div>
            <table class="table table-bordered table-striped">
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            {{ $stock_supply->supply->supply_name ?? '' }}
                        </td>
                    </tr>
                    <tr>
                        <th>
                        ...
                        <td>
                            {{ $stock_supply->supply->danger_level ?? '' }}
                        </td>
                    </tr>
                    <tr>
                        <th>
                        ...
                        <td>
                            {{ $stock_supply->current_stock }}
                        </td>
                    </tr>
                </tbody>
            </table>
            <h4 class="text-center">
                History of {{ $stock_supply->supply->supply_name }}
                @if(count($stock_supply->supply->transactions) == 0)
                    is empty
                @endif
            </h4>
            @if(count($stock_supply->supply->transactions) > 0)
                <table id="supplyDetaisTransactionTable" class="table  table-bordered table-striped display compact" style="width:75%; margin: auto;">
                    <thead>
                        <tr>
                            <th>Created At</th>
                            <th>User</th>
                        ...
                        @foreach($stock_supply->supply->transactions as $transaction)
                            <tr @if($loop->first) class="bg-info" @endif>
                                <td>{{($transaction->created_at)->isoFormat('MMMM D, YYYY (h:mm a)') }}</td>
                                <td>
...
<!-- Modal -->
<div class="modal fade" id="editStockSupply" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Stock Details of {{ $stock_supply->supply->supply_name}}</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <form method="POST" action="{{url('update-supply-stock')}}" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="id" id="stock_supply_id" />
                            <div class="form-group">
                                <label>Quantity</label>
                                <input type="number" name="current_stock" class="form-control" id="current_stock"/>
                                
                            </div>
                         
                            <div class="form-group">
                                <label>Consumed</label>
                                <input type="number" name="consumed" class="form-control" id="consumed"/>
                            ...
                            <button type="button" class="btn btn-secondary" data-dismiss="modal" style="float:right; margin-right: 10px;"><i class="fas fa-times"></i> Close</button>
                        </form>
      </div>
...
<script>
    $(document).ready(function () {
        ...
        $(document).on('click', '.editbtn', function () {
            var stock_supply_id = $(this).val();
            //alert(stock_supply);
            $('#editStockSupply').modal('show');

            $.ajax({
                type: "GET",
                url: "/edit-supply-stock/" + stock_supply_id,
                success: function (response) {
                    console.log(response);
                    $('#current_stock').val(response.stock_supply.current_stock);
                    $('#consumed').val(response.stock_supply.consumed);
                    $('#total').val(response.stock_supply.total);
                    $('#stock_supply_id').val(stock_supply_id);
                }
            });
        });
   // var table = $('# your selector').DataTable();
        var table = $('#supplyDetaisTransactionTable').DataTable({
            bSort: false,
            //dom:'lBfrtip',
                //'flrtipB',
                //select: true,
                // dom: 'Bfrtip',
           // "columnDefs": [
                //{ "width": "20%", "targets": 1 },  { "width": "5%", "targets": 5 },{ "width": "5%", "targets": 6 },
            ...
            buttons: [
                    ...
                    $(win.document.body)
                        ...
                        .prepend(
                            ...
                            '<h4 style="margin-top:20px; margin-left:480px";>List of Supplies</h4>',
                        );
        ...
        }).buttons().container().appendTo('#supplyStockTable_wrapper .col-md-6:eq(0)');

    });
</script>
@if(Session::has('stock_supply_updated'))
           <script>
              Swal.fire(
              'Good job!',
              '{!! Session::get('stock_supply_updated') !!}',
              'success'
            )
           </script>              
@endif
