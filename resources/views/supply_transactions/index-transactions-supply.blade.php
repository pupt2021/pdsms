...
<div class="card">
             <div class="card-body">
<table id="supplyTransactionTable" class="table table-bordered table-striped">
                    <thead>
<tr>
    <!--<th>#</th>-->
    <th>Created At</th>
...
<tbody>
   @foreach($supply_transactions as $key => $transaction)     
        <tr data-entry-id="{{ $transaction->id }}" @if($loop->first) class="bg-info" @endif>    
            ...
            <td>{{$transaction->supply->supply_name ?? ''}}</td>
...
<script>
   ...
   // var table = $('# your selector').DataTable();
        var table = $('#supplyTransactionTable').DataTable({
            bSort: false,
        ...
        }).buttons().container().appendTo('#supplyTransactionTable_wrapper .col-md-6:eq(0)');
